...
export function getColumn(columnId: string): LockerColumn | undefined {
  ...
  console.log(`[Locker] getColumn(${columnId}) -> ${column ? `found at ${column.ip}:${column.port}` : 'not found'}`);
  console.log(`[Locker] Registry has ${columnRegistry.size} columns: ${Array.from(columnRegistry.keys()).join(', ') || 'none'}`);
  return column;
}
...
export async function handleColumnAnnounce(data: {
  ...
  console.log(`[Locker] handleColumnAnnounce called for ${data.columnId}`);
  console.log(`[Locker] Registry before: ${columnRegistry.size} columns`);
  ...
  console.log(`[Locker] Registry after: ${columnRegistry.size} columns: ${Array.from(columnRegistry.keys()).join(', ')}`);
 ...
 * 
 * Also registers column if not known (handles hot-reload clearing registry)
 */
export async function handleHeartbeat(data: {
  ...
  let column = columnRegistry.get(data.columnId);
  
  // If column not in registry, auto-register from heartbeat data
  // This handles the case where hot-reload cleared the registry
  if (!column) {
    console.log(`[Locker] Auto-registering column ${data.columnId} from heartbeat`);
    
    // Prefer IP from body (ESP32 knows its own IP), then client IP, then DB, then default
    const dbColumn = db.prepare(`SELECT ip, port, lockerCount FROM LockerColumn WHERE id = ?`).get(data.columnId) as any;
    
    const ip = data.ip || clientIp || dbColumn?.ip || '192.168.150.3';
    const port = data.port || dbColumn?.port || 80;
    const lockerCount = dbColumn?.lockerCount || data.lockers.length;
    
    console.log(`[Locker] Body IP: ${data.ip}, Client IP: ${clientIp}, DB IP: ${dbColumn?.ip}, Using: ${ip}`);
    ...
    columnRegistry.set(data.columnId, {
      ...
      isOnline: true,
      sensors: Array(lockerCount).fill(null).map(() => ({
        doorClosed: false,
        doorOpen: false,
        irBeamClear: true,
        occupied: false,
    ...
    column = columnRegistry.get(data.columnId)!;
    console.log(`[Locker] Column ${data.columnId} auto-registered at ${ip}:${port}`);
    
    // Update database with correct IP
    if (clientIp) {
      db.prepare(`
        UPDATE LockerColumn SET ip = ?, lastSeen = ?, isOnline = 1 WHERE id = ?
      `).run(clientIp, new Date().toISOString(), data.columnId);
    }
  }
